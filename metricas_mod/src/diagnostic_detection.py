...
class DiagnosticConfig:
    ...
    max_span_distance_for_containment: int = 50
...
def diagnostic_config_from_dict(raw: dict[str, Any] | None) -> DiagnosticConfig:
    ...
    return DiagnosticConfig(
        ...
        max_span_distance_for_containment=int(raw.get("max_span_distance_for_containment", 50)),
...
def _span_distance(gold: dict[str, Any], pred: dict[str, Any]) -> int | None:
    gold_span = _span(gold, "gold")
    pred_span = _span(pred, "predicho")
    if gold_span is None or pred_span is None:
        return None
    if max(gold_span[0], pred_span[0]) < min(gold_span[1], pred_span[1]):
        return 0
    return max(gold_span[0], pred_span[0]) - min(gold_span[1], pred_span[1])
...
def _candidate_row(gold: dict[str, Any], pred: dict[str, Any], cfg: DiagnosticConfig) -> dict[str, Any] | None:
    ...
    dist = _span_distance(gold, pred)
    if containment and cfg.max_span_distance_for_containment >= 0 and dist is not None:
        if dist > cfg.max_span_distance_for_containment and not identifier_match:
            containment = False
...
def evaluate_diagnostic_detection(detail: pd.DataFrame, cfg: DiagnosticConfig) -> pd.DataFrame:
    ...
    pred_rows = detail[
        detail["tipo_resultado"].eq("extra")
        & detail["pred_id"].astype(str).ne("")
        & detail["subtipo_resultado"].ne("extra_fragmento")
    ]
    candidates = []
...
def summarize_diagnostic_detection(detail: pd.DataFrame, diagnostics: pd.DataFrame, metrics_model: pd.DataFrame | None = None, scope: str = "principal") -> pd.DataFrame:
    ...
    for model, group in detail.groupby("modelo"):
        ...
        extra_fragmento = int((group["subtipo_resultado"].eq("extra_fragmento") & _scope_pred_mask(group, scope)).sum())
        rows.append(
            {
                "modelo": model,
...
def diagnostic_invariants(detail: pd.DataFrame, diagnostics: pd.DataFrame) -> pd.DataFrame:
    ...
    for scope in ["principal", "opcional", "total"]:
        ...
        for _, row in summary.iterrows():
            ...
            diagnostic_extras = int(row["extras_asociables"]) + int(row["extras_reales"]) + int(row["extra_fragmento"])
            rows.append(
                {
                    "modelo": row["modelo"],
                    "universo": scope,
